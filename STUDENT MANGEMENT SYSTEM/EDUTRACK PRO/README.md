...
>
> 🚀 **MySQL and Spring Boot are NOT required to run this application!**
> The application runs directly in your web browser with client-side authentication, dashboard metrics, student directory, search, filters, validation, and theme settings persisted in browser `localStorage`.
>
...
> 1. In **VS Code**, right-click [`login.html`](login.html) (or `index.html`) and select **"Open with Live Server"**.
> 2. Or simply double-click [`login.html`](login.html) in your file explorer to open it in Chrome, Edge, Firefox, or any browser.
> 3. Enter the credentials (`admin` / `admin123`) to access the dashboard.
> 4. To log out, click the **Logout** button in the dashboard navbar.
>
> *(The `backend/` directory and `database.sql` script are preserved solely as reference materials and are not needed to use the web application).*
